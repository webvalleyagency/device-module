...
function extractDevices (ctx, userAgent = DEFAULT_USER_AGENT) {
  ...
  if (userAgent === 'Amazon CloudFront') {
    if(ctx.ssrContext !== undefined) {
      if (ctx.ssrContext.req.headers['cloudfront-is-mobile-viewer'] === 'true') {
        mobile = true
        mobileOrTablet = true
      }
      if (ctx.ssrContext.req.headers['cloudfront-is-tablet-viewer']['value'] === 'true') {
        mobile = false
        mobileOrTablet = true
      }
    } else {
      if (ctx.req.headers['cloudfront-is-mobile-viewer']['value'] === 'true') {
        mobile = true
        mobileOrTablet = true
      }
      if (ctx.req.headers['cloudfront-is-tablet-viewer']['value'] === 'true') {
        mobile = false
        mobileOrTablet = true
      }
    }
...
export default async function (ctx, inject) {
  const makeFlags = () => {
    let userAgent = ''
    if (typeof ctx.ssrContext !== 'undefined') {
      userAgent = ctx.ssrContext.req.headers['user-agent']
    }
    if (typeof ctx.req !== 'undefined') {
      userAgent = ctx.req.headers['user-agent']
      if(typeof userAgent === 'object') {
        userAgent = ctx.req.headers['user-agent']['value']
      }
